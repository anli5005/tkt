        ...
        async event(_p, {id}, context) {
            ...
            if (context.user) {
                const { rows } = await db.query(
                    `SELECT events.id, events.meta, events.slug, events.can_send_emails FROM event_users
                    INNER JOIN events ON event_users.event_id = events.id
                    WHERE event_users.user_id = $1 AND event_users.event_id = $2`,
                    [context.user.id, id]
                );
                if (rows.length === 1) {
                    return {...rows[0], canSendEmails: rows[0].can_send_emails};
                }
            }
        },
    ...
    Mutation: {
        async createEvent(_p, {slug, meta}, context) {
            ...
            if (!/^[a-zA-Z0-9-_+]+$/.test(slug)) {
                throw new UserInputError("Slug contains invalid characters");
            }
        ...
        async updateEventMeta(_p, {id, meta}, context) {
            await authenticate(context);
            if (!context.user) {
                throw new AuthenticationError();
            }
            ...
            const { rows } = await db.query(
                `UPDATE events SET meta = $1 FROM event_users WHERE events.id = event_users.event_id
                AND event_users.user_id = $2 AND events.id = $3
                RETURNING id, meta, slug;`,
                [meta, context.user.id, id]
            );
        ...
        async createTickets(_p, {event, tickets, send}, context) {
            ...
            if (send) {
                ...
                if (rowCount === 1) {
                    ...
                    if (!eventObj.can_send_emails) {
                        throw new ForbiddenError("This event is not allowed to send emails.");
                    }
                }
            } else {
